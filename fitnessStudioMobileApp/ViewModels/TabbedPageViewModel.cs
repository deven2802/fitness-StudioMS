...
namespace fitnessStudioMobileApp.ViewModels
{
    public partial class TabbedPageViewModel : ObservableObject, INotifyPropertyChanged
    {
        ...
        [RelayCommand]
        public async Task GoToAnnouncementPage()
        {
            await Shell.Current.GoToAsync("AnnouncementPage");
        }
        ...
        */
        ...
        {
            var firebase = new FirebaseClient("https://fitnessstudioms-b48bf-default-rtdb.asia-southeast1.firebasedatabase.app/");
            var announcements = await firebase
                .Child("Announcements")
                .OnceAsync<AnnouncementInfo>();

            AnnouncementInfo.Clear();

            foreach (var announcement in announcements)
            {
                AnnouncementInfo.Add(new AnnouncementInfo
                {
                    AnnouncementDetail = announcement.Object.AnnouncementDetail,
                    AnnouncementImage = announcement.Object.AnnouncementImage,
                });
            }
        }
        ...
        public TabbedPageViewModel()
        {
            ...
            LoadAnnouncementsAsync().ConfigureAwait(false);
